...
def draw_polygons( polygons, screen, color ):
    triangles = int(len(polygons)/3)
    for tri in range(triangles):
        if calculate_normal(polygons, tri * 3)[2] > 0:
            p0 = tri * 3
            p1 = p0 + 1
            p2 = p1 + 1
            draw_line(int(polygons[p0][0]), int(polygons[p0][1]),
                      int(polygons[p1][0]), int(polygons[p1][1]),
                      screen, color
            )
            draw_line(int(polygons[p0][0]), int(polygons[p0][1]),
                      int(polygons[p2][0]), int(polygons[p2][1]),
                      screen, color
            )
            draw_line(int(polygons[p2][0]), int(polygons[p2][1]),
                      int(polygons[p1][0]), int(polygons[p1][1]),
                      screen, color
            )
...
def add_torus(polygons, cx, cy, cz, r0, r1, steps ):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            if (longt == (steps - 1)):
                p0 = index
                p1 = (index + steps) % len(points)
                p2 = (lat * steps + steps) % len(points)
                add_polygon(polygons,
                        points[p0][0], points[p0][1], points[p0][2],
                        points[p1][0], points[p1][1], points[p1][2],
                        points[p2][0], points[p2][1], points[p2][2]
