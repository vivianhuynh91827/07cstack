...
The file follows the following format:
     ...
     push: push a copy of the current top of the coordinatte system stack onto the cs stack
...
def parse_file( fname, edges, polygons, csystems, screen, color ):
    ...
    while c < len(lines):
        ...
        if line == 'sphere':
            add_sphere(polygons, float(args[0]), float(args[1]), float(args[2]), float(args[3]), steps_3d)
            matrix_mult(csystems[len(csystems) - 1], polygons)
            draw_polygons(polygons, screen, color)
            polygons = []

        elif line == 'torus':
            add_torus(polygons, float(args[0]), float(args[1]), float(args[2]), float(args[3]), float(args[4]), steps_3d)
            matrix_mult(csystems[len(csystems) - 1], polygons)
            draw_polygons(polygons, screen, color)
            polygons = []

        elif line == 'box':
            add_box(polygons, float(args[0]), float(args[1]), float(args[2]), float(args[3]), float(args[4]), float(args[5]))
            matrix_mult(csystems[len(csystems) - 1], polygons)
            draw_polygons(polygons, screen, color)
            polygons = []

        elif line == 'circle':
            add_circle(edges, float(args[0]), float(args[1]), float(args[2]), float(args[3]), steps_2d)
            matrix_mult(csystems[len(csystems) - 1], edges)
            draw_lines(edges, screen, color)
            edges = []

        elif line == 'hermite' or line == 'bezier':
            add_curve(edges, float(args[0]), float(args[1]), float(args[2]), float(args[3]), float(args[4]), float(args[5]), float(args[6]), float(args[7]), steps_2d, line)
            matrix_mult(csystems[len(csystems) - 1], edges)
            draw_lines(edges, screen, color)
            edges = []
